...
export function HabitsWidget() {
        ...
        return (
            <ContextMenu key={habit.id}>
                    ...
                    >
                        ...
                        {isScheduled && (
                            <span className={cn(
                                ...
                                {habit.daysOfWeek!.map((d: number) => ['Su', 'M', 'Tu', 'W', 'Th', 'F', 'Sa'][d]).join(', ')}
                            </span>
                        )}
                ...
                <ContextMenuContent>
                    ...
                    <ContextMenuItem onClick={() => promoteToQueue(habit.title, true)} className="text-orange-500 focus:text-orange-500">
    ...
    return (
        <div className="w-full mb-6 relative group space-y-4">
            ...
            <Button 
                variant="ghost" 
                size="icon" 
                className="absolute -top-1 right-1 rounded-full h-6 w-6 shrink-0 text-muted-foreground/50 hover:text-foreground hover:bg-neutral-100 dark:hover:bg-neutral-900 z-10 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity"
                onClick={() => setIsManagerOpen(true)}
            >
                    ...
                    <form onSubmit={onCreate} className="flex flex-col gap-2">
                        <div className="flex gap-2 relative">
                            ...
                            {newTitle.trim().length > 0 && (
                                <div className="absolute right-[88px] top-1/2 -translate-y-1/2 pointer-events-none">
                                    <span className={cn(
