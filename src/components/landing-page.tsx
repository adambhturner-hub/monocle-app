...
import { Target, ListTodo, Zap, CheckCircle2, Shuffle, Repeat, X, Pause, Music, Check, MoreHorizontal, ChevronUp, Timer, Dices, Play, Activity } from 'lucide-react';
...
export function LandingPage({ onGoogleSignIn, onSendMagicLink, isSubmitting }: LandingPageProps) {
    ...
    return (
        <div className="min-h-screen bg-background text-foreground selection:bg-primary/30 flex flex-col font-sans overflow-x-hidden">
            ...
            <header className="px-6 py-6 md:px-12 md:py-8 flex justify-between items-center w-full max-w-7xl mx-auto">
                ...
                <div className="flex items-center gap-4">
                    <Button
                        ...
                        onClick={() => {
                            setShowEmailInput(false); // Reset to show both options
            ...
            <main className="flex-1 flex flex-col items-center justify-center px-6 pt-16 pb-24 md:pt-32 md:pb-40 text-center relative max-w-5xl mx-auto w-full">
                ...
                <div ref={optionsRef} className="flex flex-col items-center gap-4 min-h-[140px] justify-center">
                    {!showEmailInput ? (
                        <>
                            ...
                            <Button
                                variant="ghost"
                                className="mt-2 h-10 px-4 text-sm font-medium text-muted-foreground hover:text-foreground flex items-center gap-2"
                        ...
                        <div className="flex flex-col items-center gap-4 max-w-sm text-center animate-in fade-in zoom-in-95 duration-500">
                            <div className="w-16 h-16 rounded-full bg-emerald-500/10 flex items-center justify-center text-emerald-500 mb-2 border border-emerald-500/20 shadow-lg shadow-emerald-500/10">
                                <CheckCircle2 className="w-8 h-8" />
                            </div>
                            <div>
                                <h3 className="text-2xl font-bold tracking-tight mb-2">Check your inbox</h3>
                                <p className="text-muted-foreground text-base px-4">
                                    We sent a magic link to <span className="font-semibold text-foreground">{email}</span>. Click it to log in.
                                </p>
                            </div>
                        </div>
                    ) : (
                        <form onSubmit={handleEmailSubmit} className="flex flex-col items-center gap-4 w-full max-w-md animate-in fade-in zoom-in-95 duration-300">
                            <div className="flex w-full items-center gap-2">
                                <Input
                                    ...
                                    className="h-14 rounded-full px-6 text-lg border-2 shadow-sm bg-background/50 backdrop-blur-sm flex-1"
                                    disabled={isSubmitting}
                                    autoFocus
                                />
                                <Button type="submit" size="lg" className="h-14 rounded-full px-8 font-bold shadow-xl shadow-primary/20 shrink-0" disabled={isSubmitting || !email}>
                                    {isSubmitting ? <Activity className="w-5 h-5 animate-spin" /> : 'Send Link'}
                                </Button>
                            </div>
                            <Button variant="ghost" type="button" className="text-sm text-muted-foreground hover:text-foreground" onClick={() => setShowEmailInput(false)}>
            ...
            <section className="bg-background border-t py-32 md:py-48 px-6 md:px-12 relative overflow-hidden flex flex-col items-center justify-center text-center">
                ...
                <div className="flex flex-col items-center gap-6 mt-8 min-h-[140px] justify-center">
                    {!showEmailInput ? (
                        <div className="flex flex-col sm:flex-row flex-wrap items-center justify-center gap-4">
                            <Button
                                size="lg"
                            ...
                            <Button
                                size="lg"
                                variant="outline"
                                className="h-16 md:h-20 px-8 md:px-10 text-xl md:text-2xl rounded-full font-black tracking-tight shadow-sm flex items-center gap-3 transition-all hover:scale-105 active:scale-95 border-2 border-foreground/10 hover:border-foreground/20 bg-background/50 backdrop-blur-sm"
                                onClick={() => {
                        ...
                        null /* State handled by the top block, bottom just scrolls to top on email click */
                    )}
                </div>
            </section>
